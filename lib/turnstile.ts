 ...
 * corpo atravessa inteiro para a Cloudflare: a auditoria de segurança mediu 500.000
 ...
 * fail-open silencioso, a mesma classe de falha que a  matou no caminho
 * de e-mail, e é o achado R1 da auditoria de segurança do PR #105.
 *
...
/**
 * Configuração inválida em runtime: secret ausente, ou chave de
 * TESTE em Production. NÃO é o mesmo que token reprovado: é defeito nosso, e o
 * padrão da  vale aqui — nada se assume, falha alto e visível para quem
 * opera.
 */
 ...
 *
 * INDISPONIBILIDADE, decisão explícita: com o Turnstile fora, a inscrição
 ...
 * @throws {TurnstileUnavailableError} siteverify fora do ar (indisponibilidade)
 */
export async function verifyTurnstileToken(token: unknown): Promise<TurnstileVerdict> {
 ...
 /*
    chave de teste em Production é defeito de CONFIGURAÇÃO, e é
    tratado como tal: mesmo caminho do secret ausente, mesmo 503 indisponibilidade,
    ...
    mantém fora da categoria de resposta distinguível.
